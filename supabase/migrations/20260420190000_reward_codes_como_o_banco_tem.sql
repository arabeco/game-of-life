-- OS CODIGOS DE RESGATE, COMO O BANCO REALMENTE TEM.
--
-- Este arquivo vivia em `sql/reward_codes_v1.sql`, fora das migracoes, e era a
-- unica descricao fiel da tabela que esta no ar. Ao lado dele existia a migracao
-- `20260420203000_create_reward_code_system.sql`, que descrevia a MESMA tabela
-- com outros nomes: `grants` no lugar de `reward_payload`, `expires_at` no lugar
-- de `ends_at`, `max_redemptions_per_user` no lugar de `per_user_limit`.
--
-- As duas comecavam com `create table if not exists`, entao a primeira que
-- rodasse venceria e a outra nao reclamaria de nada. Venceu esta — conferido em
-- 22/09/2026 lendo o information_schema.
--
-- A OUTRA ERA UMA BOMBA. Ela nunca foi aplicada (nao consta em
-- supabase_migrations.schema_migrations), mas continuava no repositorio: um
-- `db push` faria o `create table` virar no-op e o `create or replace function`
-- substituir o `redeem_reward_code` que funciona por um que le `grants`, coluna
-- que nao existe. Todo resgate passaria a estourar, e ninguem descobriria sem
-- alguem tentar usar um codigo.
--
-- Ela foi apagada e esta virou migracao, para que o repositorio tenha uma
-- verdade so e para que um banco novo nasca igual ao que esta no ar. Tudo aqui e
-- idempotente: `if not exists` na tabela e nos indices, `create or replace` na
-- funcao.
