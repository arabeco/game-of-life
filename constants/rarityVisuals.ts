...
export const RARITY_VISUALS: Record<VisualRarity, RarityVisual> = {
    ...
    quest: { key: 'quest', label: 'Quest', hex: '#14B8A6', rgb: '20,184,166' },
};
...
export const normalizeVisualRarity = (value?: string | null): VisualRarity | undefined => {
    ...
    if (lower === 'rare' || lower === 'raro') return 'rare';
    if (lower === 'epic' || lower === 'épico' || lower === 'epico') return 'epic';
    if (lower === 'legendary' || lower === 'lendário' || lower === 'lendario') return 'legendary';
    if (lower === 'quest' || lower.includes('clã') || lower.includes('clan') || lower.includes('season')) return 'quest';
    return undefined;
};
...
export const getChestVisual = (type: ChestType | string): RarityVisual => {
    ...
    if (lower.includes('raro') || lower.includes('ciclo')) return RARITY_VISUALS.rare;
    if (lower.includes('épico') || lower.includes('epico') || lower.includes('radiante')) return RARITY_VISUALS.epic;
    if (lower.includes('lendário') || lower.includes('lendario') || lower.includes('legendary') || lower.includes('season')) return RARITY_VISUALS.legendary;
    return RARITY_VISUALS.common;
};
