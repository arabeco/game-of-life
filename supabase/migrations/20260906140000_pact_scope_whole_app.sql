-- A missao individual ganha um segundo escopo: o APP INTEIRO.
--
-- Ate aqui ela era sempre de uma arena. "Conclua 20 acoes" vivia noutro lugar,
-- com outro caminho de pagamento e outro slot — duas coisas parecidas, dois
-- encanamentos, e dava para estar com as duas ao mesmo tempo.
--
-- Como fica: arena NULA significa "o app inteiro". Nao precisou de coluna nova,
-- porque a ausencia da arena ja e a informacao, e ela e verdade dos dois lados —
-- quem mede, mede sem filtrar por arena.
--
-- O que muda aqui:
--   1. o slot passa a ser medido pelo TIPO (arena_pact_kind), nao pela arena,
--      que agora pode ser nula com uma missao de pe;
--   2. a existencia da arena so e exigida quando ha arena;
--   3. a contagem de volume ignora o filtro de arena no escopo do app;
--   4. o marcador de idempotencia e a trava de janela paga usam 'sistema' como
--      chave quando nao ha arena.
--
-- So o molde de VOLUME aceita o escopo do app. Constancia sem arena viraria a
-- sequencia global que acabou de ser removida; conclusao e retomada nao querem
-- dizer nada sem uma frente especifica. As duas RPCs recusam.
--
-- Nao altera assinatura, EXP, nota de ciclo nem recompensa ja paga.
-- Conferido antes de escrever: 0 missoes individuais ativas, 0 janelas pagas.
--
