...
invite_only as (
  select
    au.id as id,
    au.id as user_id,
    au.email as tracking_email,
    up.nickname as tracking_nickname,
    null::text as full_name,
    null::text as whatsapp,
    'golden_invite_auto'::text as source,
    'ouro'::text as cohort_label,
    'ouro'::text as beta_tier,
    'invited'::text as stage,
    claimed.code as invite_code,
    null::timestamptz as first_contact_at,
    claimed.claimed_at as invited_at,
    null::timestamptz as onboarding_call_scheduled_at,
    null::timestamptz as onboarding_call_completed_at,
    null::timestamptz as observation_started_at,
    null::timestamptz as observation_ends_at,
    null::timestamptz as first_value_at,
    null::integer as first_value_minutes,
    null::timestamptz as activated_at_override,
    null::boolean as d2_returned_override,
    null::boolean as d7_returned_override,
    null::boolean as cycle_started_override,
    null::boolean as cycle_closed_override,
    null::integer as active_days_14d,
    null::integer as tasks_completed_14d,
    null::text as where_stuck,
    null::text as what_confused,
    null::text as what_made_them_return,
    null::text as qualitative_feedback,
    null::text as ceo_notes,
    null::timestamptz as next_follow_up_at,
    false as ignore_in_marco1,
    null::text as ignore_reason,
    coalesce(claimed.claimed_at, up.created_at, au.created_at) as created_at,
    coalesce(claimed.claimed_at, up.created_at, au.created_at) as updated_at,
    up.email as profile_email,
    up.nickname as profile_nickname,
    up.role as profile_role,
    up.created_at as profile_created_at,
    ...
    claimed.claimed_at as invite_claimed_at
  from auth.users au
  join public.golden_invites claimed
    on claimed.claimed_by_user_id = au.id
   and coalesce(claimed.is_used, false) = true
   and lower(coalesce(claimed.code, '')) like 'ouro-%'
  left join public.user_profiles up
    on up.id = au.id
  left join public.marco1_beta_tracking bt
    on bt.user_id = au.id
    or (
      bt.user_id is null
      and bt.email is not null
      and lower(bt.email) = lower(au.email)
    )
  where bt.id is null
    and lower(coalesce(up.role, 'user')) not in ('gm', 'admin', 'admin_gm')
),
tracked as (
  select * from tracked_manual
  union all
  select * from invite_only
),
arena_stats as (
  select
    user_id,
