...
export const LegacyProjectionScene: React.FC<LegacyProjectionSceneProps> = ({
    ...
    return (
        <section id={id} className={sceneClass}>
            ...
            >
                <div className="legacy-scene-wall" />
                <div className="legacy-scene-column legacy-scene-column-left" />
                <div className="legacy-scene-column legacy-scene-column-right" />
                <div className="legacy-scene-sconce legacy-scene-sconce-left" />
                <div className="legacy-scene-sconce legacy-scene-sconce-right" />
                <div className="legacy-scene-altar" />
            </div>
            ...
            <div className={`relative z-10 flex min-h-full flex-col ${interactive ? 'px-4 pb-5 pt-6 sm:px-6 sm:pb-6 sm:pt-7' : ''}`}>
                <div
                    className={`mx-auto transition-all duration-700 ${timelineVisible ? 'translate-y-0 opacity-100' : 'pointer-events-none -translate-y-4 opacity-40'} ${enteringProjection ? 'scale-[0.985]' : plaqueReclaimingFocus ? 'scale-[1.02]' : 'scale-100'} ${interactive ? 'cursor-pointer' : ''}`}
                    onClick={interactive ? onActivatePlaque : undefined}
                    style={{
                        maxWidth: interactive ? 'min(74vw, 520px)' : undefined,
                        boxShadow: plaqueReclaimingFocus
                            ? `0 -6px 45px ${backdropSkin.edgeGlow}, inset 0 0 100px rgba(212,175,55,0.08)`
                            : eraTransitionPulse
                ...
                >
                    <div
                        ...
                        style={{
                            boxShadow: plaqueReclaimingFocus
                                ? `0 0 70px ${backdropSkin.edgeGlow}, inset 0 0 70px rgba(212,175,55,0.08)`
                                : eraTransitionPulse
                                    ? `0 0 40px ${eraTransitionPulse}55, inset 0 0 60px ${eraTransitionPulse}22`
                                    : undefined,
                        }}
                    >
                        <LegacyGrandPlaque eras={eras} sovereignName={sovereignName} compact={!interactive} />
                    </div>
                </div>

                <div className={`mt-auto space-y-4 transition-all duration-700 ${plaqueReclaimingFocus ? 'scale-[0.97] opacity-35 blur-[1px]' : 'scale-100 opacity-100'}`}>
                    <div className={`mx-auto max-w-[min(94vw,1120px)] legacy-identity-rail transition-all duration-500 ${identityPulse ? 'shadow-[0_0_38px_rgba(212,175,55,0.16)]' : ''}`}>
                        <div className={`legacy-identity-avatar transition-all duration-500 ${identityPulse ? 'ring-2 ring-[var(--skin-accent-color)]/40 shadow-[0_0_26px_rgba(212,175,55,0.18)]' : ''}`}>
                            {activeIdentity.avatarUrl ? (
                                ...
                                <div className="flex h-full w-full items-center justify-center text-lg font-black tracking-[0.18em] text-white/85">{avatarInitials}</div>
                            )}
                        </div>
                        <div className="legacy-identity-copy">
                            <p className="legacy-kicker legacy-kicker-muted">Soberano</p>
                            <h3 className={`mt-2 text-xl font-black tracking-tight text-white transition-all duration-500 ${identityPulse ? 'text-[var(--skin-accent-color)]' : ''}`}>{activeIdentity.nickname || sovereignName}</h3>
                        </div>
                        <div className="legacy-identity-stat">
                            <p className="legacy-kicker legacy-kicker-muted">Patente</p>
                            <p className="mt-2 text-sm font-black text-white">{activePatent}</p>
                        </div>
                        <div className="legacy-identity-stat">
                            <p className="legacy-kicker legacy-kicker-muted">Cla</p>
                            <p className="mt-2 text-sm font-black text-white">{activeClan}</p>
                        </div>
                        <div className="legacy-identity-stat">
                            <p className="legacy-kicker legacy-kicker-muted">Nivel</p>
                            <p className="mt-2 text-sm font-black text-white">{activeIdentity.level}</p>
                        </div>
                    </div>

                    <div className={`mx-auto transition-all duration-1000 ${timelineVisible ? 'translate-y-0 opacity-100' : 'pointer-events-none translate-y-6 opacity-0'}`} style={{ maxWidth: interactive ? 'min(96vw, 1480px)' : undefined }}>
                                            ...
                                            <div className="space-y-4">
                                                ...
                                                <div className="flex gap-4">
                                                    {(era.cycles || []).map((cycle, cycleIndex) => {
                                                        ...
                                                        const baseClass = `w-[304px] shrink-0 rounded-[26px] border bg-black/30 p-4 text-left shadow-[0_18px_30px_rgba(0,0,0,0.28)] transition-all ${interactive ? 'hover:-translate-y-1' : ''} ${isFocused ? 'ring-2 ring-[var(--skin-accent-color)]/45 scale-[1.01] opacity-100' : 'opacity-80'}`;
                                                        ...
                                                        const cycleContent = (
                                                            <>
                                                                <MetalReportCard
                                                                    ...
                                                                    compact
                                                                />
                                                                <div className="mt-3 rounded-[22px] border border-white/10 bg-black/28 p-3">
                                                                    <p className="legacy-kicker legacy-kicker-muted">Planner condensado</p>
                                                                    <MiniCyclePlannerSnapshot weeks={cycle.weeklyAtlas || []} accentColor={skin.edge} className="mt-3" compact />
                                                                </div>
                                                            </>
                                                        );
