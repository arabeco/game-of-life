...
export const CampaignsCodex: React.FC<CampaignsCodexProps> = ({ onClose, initialCampaignId }) => {
    ...
    const handleCreateCampaign = async () => {
        ...
        const createdCampaign = await addCampaign({
            title,
            description: 'Descri��o da campanha...',
            type: 'sequential',
            arenaIds: [],
            arenaConfig: {},
            priority: 'media',
    ...
    const handleDeleteCampaign = () => {
        if (!selectedCampaign) return;
        if (confirm('Tem certeza que deseja excluir esta campanha? TODAS as arenas e a��es dentro dela ser�o exclu�das permanentemente.')) {
    ...
    const handleRemoveArena = (arenaId: string, e: React.MouseEvent) => {
        ...
        if (isCodexCampaign) {
            alert("N�o � poss�vel remover arenas de uma campanha de Codex.");
            return;
        }
