...
export const PoolAction: React.FC<PoolActionProps> = ({ action, count, isUnlimited, taskId, onComplete, onCustomDragStart, onActionClick }) => {
    ...
    return (
        <div
            ...
            className={`
                h-full aspect-square border border-[var(--accent-bronze)]/50 rounded-xl flex items-center justify-center p-1 flex-shrink-0 relative transition-all duration-300
                ${isDisabled ? 'border-gray-700' : 'cursor-grab active:cursor-grabbing hover:border-[var(--skin-accent-color)]/50 hover:scale-105 active:scale-95'}
            `}
        >
            <span className={`text-lg ${isHolding ? 'scale-110' : ''} transition-transform`}>{action.icon}</span>

            {!clanProgressDisplay && (isUnlimited || count >= 1) && (
                <div className={`absolute -top-1 -right-1 ${count <= 0 ? 'bg-gray-500/80' : 'bg-[var(--bronze)]'} text-black text-[9px] font-black rounded-full min-w-[14px] h-[14px] px-0.5 flex items-center justify-center shadow-[0_0_10px_rgba(0,0,0,0.5)] z-20 border border-black/20`}>
                    {isUnlimited ? '∞' : `${count}`}
                </div>
            )}
            ...
            {isTransitioning && (
                <div className="absolute inset-0 bg-green-500/20 rounded-xl overflow-hidden pointer-events-none">
                    <div className="h-full bg-green-500/40 animate-progress-fill origin-left" />
                </div>
            )}

            {isHolding && (
                <div className="absolute inset-0 bg-black/50 rounded-xl animate-pulse pointer-events-none">
