...
import {
    ...
    getOracleChatNotificationsForProfile,
    getOracleRequestNotificationsForProfile,
...
const NotificationsList: React.FC<NotificationsListProps> = ({ notifications, onRead, onDelete, oracleMode, onClose }) => {
            ...
            {selectedNotification && (
                <NotificationDetailModal
                    ...
                    onNavigate={(notification) => {
                        navigateFromNotification(notification, onClose);
...
}> = ({ notification, oracleMode, onClose, onRead, onDelete, onNavigate }) => {
    const lane = getNotificationLane(notification.type);
    const label = getNotificationLabel(notification.type);
    const laneLabel = getNotificationLaneLabel(notification.type);
    const icon = getNotificationIcon(notification.type);
                        ...
                        <div className="flex items-center gap-2">
                            {actionLabel && (
                                <button
                                    ...
                                    className="rounded-full border border-[var(--skin-accent-color)]/25 bg-[var(--skin-accent-color)]/12 px-3 py-2 text-[10px] font-black uppercase tracking-[0.2em] text-[var(--skin-accent-color)] transition-colors hover:bg-[var(--skin-accent-color)]/18"
                                >
                                    {actionLabel}
                                </button>
                            )}
...
const navigateFromNotification = (notification: Notification, onClose: () => void) => {
    if (notification.type === 'direct_message') {
        dispatchAppView({ view: 'social' });
        window.dispatchEvent(new CustomEvent('mundo-tab-request', {
            detail: {
                tab: 'social',
                socialSection: 'messages',
                participantId: notification.metadata?.participantId || notification.metadata?.senderId || null,
            },
        }));
        onClose();
        return;
