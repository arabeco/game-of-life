-- A escada dos baus: um degrau por bau, e nunca para baixo.
--
-- Duas mudancas.
--
-- 1. NUNCA ABAIXO DO PROPRIO TIER.
--    O bau Comum dava tier 4 em 2% das vezes e o Incomum nunca dava — o mais
--    barato era, as vezes, melhor que o seguinte. Pior: Comum e Incomum
--    entregavam praticamente a mesma coisa (tier medio 1,27 contra 1,35), e a
--    maior distancia da escada inteira ficava entre o Incomum e o Raro, que e
--    exatamente onde o Incomum deveria estar.
--
--    Agora cada bau ancora no proprio tier com 80% e so vaza para CIMA:
--
--      Comum      t1 80  t2 15  t3 5     media 1,25
--      Incomum    t2 80  t3 15  t4 5     media 2,25
--      Raro       t3 80  t4 15  t5 5     media 3,25
--      Epico      t4 80  t5 20           media 4,20
--      Lendario   t5 100                 media 5,00
--      Mitico     t6 100                 media 6,00
--
--    Epico e Lendario nao vazam ate o tier 6 porque mitico e exclusivo de
--    temporada, e bau normal filtra exclusivo de temporada fora. Enquanto nao
--    existir item tier 6 fora de temporada, o Lendario e teto fechado.
--
--    Consequencia aceita: quem ja tem todo o tier 1 recebe duplicata no Comum.
--    E o certo — o bau mais barato deixa de ser interessante quando voce passou
--    dele. O servidor ja sorteia primeiro entre o que a pessoa nao tem.
--
-- 2. O TEMA DE INTERFACE SAI DO BAU MITICO.
--    A colecao de temporada tem cinco pecas: skin, borda, banner, insignia e
--    tema. O bau sorteia as TRES que se veste — skin, borda e banner. O selo
--    entrega as outras duas: a insignia da temporada e o tema.
--
--    A conta fecha sozinha e sem sorte: sao exatamente tres baus miticos (um
--    por jornada), o servidor sorteia primeiro o que a pessoa NAO tem, e o pool
--    tem exatamente tres pecas. Quem completa a temporada completa a colecao.
--
--    Por que o tema no fim e nao o banner: o selo ja entrega a insignia, que
--    mora no perfil. Banner junto seria duas pecas no mesmo lugar, e quem
--    fechasse a temporada abriria o app no dia seguinte sem ver diferenca. O
--    tema repinta o app inteiro.
--
-- O resto da funcao e copia de 20260903120000: so o `case` das curvas e os dois
-- filtros de temporada mudam.
...
begin
  ...
  begin
    if v_guaranteed_tier > 0 then
      ...
      case v_chest_type
        -- Nunca ABAIXO do proprio tier. Um bau Raro que entrega item comum
        -- decepciona duas vezes: pelo item e pela promessa. Agora cada bau
        -- ancora no tier dele (80%) e so vaza para CIMA (15% + 5%).
        when 'comum' then
          if v_rand < 80 then v_dropped_tier := 1;
          elsif v_rand < 95 then v_dropped_tier := 2;
          else v_dropped_tier := 3; end if;
        when 'incomum' then
          if v_rand < 80 then v_dropped_tier := 2;
          elsif v_rand < 95 then v_dropped_tier := 3;
          else v_dropped_tier := 4; end if;
        -- 'ciclo' e legado: nada mais o concede. Ele veste a arte do Raro,
        -- entao entrega como um Raro para quem ainda tiver um guardado.
        when 'ciclo' then
          if v_rand < 80 then v_dropped_tier := 3;
        ...
        -- Epico e Lendario nao alcancam o tier 6: mitico e exclusivo de
        -- temporada, e bau normal filtra exclusivo de temporada fora. Por isso
        -- o Epico so vaza um degrau e o Lendario nao vaza.
        when 'epico' then
          if v_rand < 80 then v_dropped_tier := 4;
          else v_dropped_tier := 5; end if;
        when 'lendario' then
          v_dropped_tier := 5;
        when 'season' then
  ...
  where i.tier = v_dropped_tier
    ...
    -- O TEMA DE INTERFACE fica fora do bau: ele e o premio de fechar a
    -- temporada, entregue pelo selo junto da insignia. Sorteavel, deixaria de
    -- ser premio. O bau sorteia as tres pecas que se veste: skin, borda e
    -- banner.
    and (v_chest_type <> 'season' or coalesce(i.season_slot, '') not in ('ui_skin', 'insignia'))
    and not exists (
      select 1
  ...
  v_bonus_fragments := case v_chest_type
    ...
    when 'lendario' then floor(random() * (350 - 150 + 1) + 150)
