...
for item in legados:
    ...
    content = content.replace('$quoteText = "Entre o estímulo e a resposta`nhá um espaço..."', '$quoteText = ' + ps_string(item["Quote"]))
    content = content.replace('$supportCore = "O psiquiatra que sobreviveu..."', '$supportCore = ' + ps_string(item["Support"]))
    content = content.replace('$analysis1 = "Viktor Frankl foi um psiquiatra..."', '$analysis1 = ' + ps_string(item["Analysis1"]))
    content = content.replace('$analysis1Close = "Ele saiu do horror..."', '$analysis1Close = ' + ps_string(item["Analysis1Close"]))
    content = content.replace('$analysis2Body = "Porque sobreviver..."', '$analysis2Body = ' + ps_string(item["Analysis2"]))
    content = content.replace('$analysis2Close = "Ele provou..."', '$analysis2Close = ' + ps_string(item["Analysis2Close"]))
...
for item in fundamentos:
    ...
    content = content.replace('$coverSupport = "A forma como você para`npode estar te drenando."', '$coverSupport = ' + ps_string(item["Support"]))
    content = content.replace('$logic1Title = "Descanso que entorpece`nnão restaura."', '$logic1Title = ' + ps_string(item["Logic1Title"]))
    content = content.replace('$logic1Body = "Quando o descanso vira..."', '$logic1Body = ' + ps_string(item["Logic1Body"]))
    content = content.replace('$logic1Close = "Entorpecer não é recuperar."', '$logic1Close = ' + ps_string(item["Logic1Close"]))
    content = content.replace('$logic2Title = "Recuperação real`ndevolve comando."', '$logic2Title = ' + ps_string(item["Logic2Title"]))
    content = content.replace('$logic2Body = "Silêncio, sono..."', '$logic2Body = ' + ps_string(item["Logic2Body"]))
    content = content.replace('$logic2Close = "Descanso real devolve presença."', '$logic2Close = ' + ps_string(item["Logic2Close"]))
    write_bom(SCRIPTS / f"generate-{folder}.ps1", content)
