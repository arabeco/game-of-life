...
export const GameProvider: React.FC<{ children: ReactNode, session: Session | null }> = ({ children, session }) => {
    ...
    const maybeNotifyVillageDuty = (userId: string) => {
        ...
        showToast(`Dever Cumprido: Ordem da Aldeia ativa (+${percent}% de býnus de EXP).`, 'success');
    ...
    useEffect(() => {
        ...
        (window as any).cleanOrphans = async () => {
            ...
            if (!uid) {
                console.error("L Usuário não autenticado.");
                return;
            }
            ...
            if (tErr || aErr || arErr) {
                console.error("L Erro ao buscar dados para limpeza:", tErr, aErr, arErr);
                return;
            }

            if (!allTasks || !allActions || !allArenas) {
                console.log("  Dados insuficientes para limpeza.");
                return;
            }
            ...
            allTasks.forEach(task => {
                ...
                if (!task.action_id) {
                    console.log(`= Tarefa ${task.id} -> Action ID nulo/vazio.`);
                ...
                if (!validActionIds.has(task.action_id)) {
                    console.log(`= Tarefa ${task.id} (Action ${task.action_id}) -> Ação não existe.`);
                    tasksToDelete.push(task.id);
                    return;
                }
                ...
                if (arenaId && !validArenaIds.has(arenaId)) {
                    console.log(`= Tarefa ${task.id} (Arena ${arenaId}) -> Arena não existe.`);
                    tasksToDelete.push(task.id);
                    return;
                }
            });
            ...
            if (tasksToDelete.length > 0) {
                ...
                for (let i = 0; i < tasksToDelete.length; i += 100) {
                    ...
                    if (error) {
                        console.error("L Erro ao deletar lote:", error);
                    } else {
        ...
        for (const level of template.levels) {
            ...
            for (const action of level.actions) {
                const { error: actionError } = await supabase.from('actions').insert({
                    ...
                    icon: action.icon || '', // Default icon if missing
    ...
    const endCycle = (currentAssets: Asset[], currentActions: Action[]): EndCycleResult => {
        ...
        // maxStreak (consecutive days with e1 completed task)
