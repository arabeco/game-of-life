# PACTO DE SOBERANIA — LEGAL

## A Experiência de Primeiro Login

**A Visualização**

- **O Vazio:** A tela inicia em Preto Absoluto (#000000).
- **A Escrita Sagrada:** O texto não “pula” na tela; ele surge com um efeito de typewriter (máquina de escrever) lento, em Dourado Metálico (#D4AF37).
- **O Ritmo:** Cada cláusula surge apenas quando a anterior termina, e a pessoa precisa avançar para seguir, criando um senso de leitura obrigatória e ritualística.
- **O Selo:** Ao final, em vez de um botão “OK”, surge um Selo de Cera ou uma Assinatura de Sangue/Luz que o usuário deve pressionar por 2 segundos (Long Press) para confirmar.

## 📜 O Texto: Pacto de Soberania

### O DESPERTAR DO SOBERANO

Para acessar a interface, você deve aceitar os termos do pacto que regem este domínio.

**I. PROPRIEDADE ABSOLUTA**  
Seus dados são sua soberania. O Life OS é “Local First”: anotações, diários e registros residem no seu dispositivo. A nuvem é apenas o seu espelho de segurança. Nós não mineramos sua vida.

**II. O VÍNCULO DE MENTORIA**  
Ao aceitar um Mentor, você autoriza a visualização parcial do seu progresso. Seus diários privados permanecem ocultos. O Life OS não se responsabiliza por orientações de terceiros; você é o único executor de suas ações.

**III. ISENÇÃO DE RESPONSABILIDADE**  
Este sistema é uma ferramenta de autogestão. Não somos médicos, terapeutas ou consultores financeiros. O risco da execução física, mental ou financeira de qualquer Codex é inteiramente do Soberano.

**IV. DIREITO AO EXÍLIO**  
A qualquer momento, você pode incinerar seus dados. O comando “Deletar Conta” é definitivo e apaga sua existência em nossos servidores, sem rastro ou recuperação.

[ PRESSIONE PARA SELAR O PACTO ]

## 🧠 Resumo dos Insights (Foco em Compliance & Retenção)

Como removemos os insights 1 e 2, focamos na relação entre Mentor e Legado:

- **A Cláusula de Não-Agência (Proteção):** O texto deixa claro que o Mentor é um “terceiro”. Isso reduz risco de processos caso um Mentor dê um conselho ruim ou tente alegar vínculo empregatício com o app.

- **O Protocolo de Legado (Dica Bônus):** O app se posiciona como algo que vale a pena ser guardado. A ideia do “Herdeiro de Soberania” (dar acesso aos dados após inatividade longa) transforma o software em um ativo familiar, aumentando absurdamente o valor percebido do plano Premium.

## ✅ Checklist de Implementação dos Termos

- [x] Tela do pacto com avanço por cláusula e selo de aceite
- [x] Efeito de escrita progressiva no texto do pacto
- [x] Armazenamento local dos dados do usuário (Local First)
- [x] Sincronização com nuvem (Supabase) como espelho de segurança
- [ ] Controle fino de visibilidade na mentoria (progresso parcial, diários privados ocultos)
- [ ] Registro de aceite do pacto (data/hora e versão do texto)
- [ ] Comando “Deletar Conta” com exclusão definitiva e irrecuperável
