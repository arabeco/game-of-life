-- O bau Mitico so entrega itens da temporada em que foi aberto.
--
-- Ate aqui o sorteio filtrava por is_season_exclusive e mais nada, entao abrir
-- um Mitico durante o Genesis podia devolver um item da Aurora - uma temporada
-- que ainda nao aconteceu. Agora a chave da temporada ativa vem do cliente,
-- que e quem a resolve por data, e o sorteio exige que o item pertenca a ela.
--
-- As duas ultimas redes de seguranca tambem passam a ignorar o bau de
-- temporada. Elas existem para nunca deixar um bau sem premio, mas no Mitico
-- faziam justamente o contrario do combinado: entregavam item de fora da
-- colecao. Se a colecao da temporada acabar, o bau falha e avisa, em vez de
-- mentir.
--
-- p_season_key tem default nulo: chamada antiga continua funcionando e cai no
-- comportamento anterior, sem filtro de temporada.
