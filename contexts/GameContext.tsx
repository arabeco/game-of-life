...
const normalizeActionFromDbRow = (row: any): Action => {
    ...
    const schedule = (action.context && typeof action.context === 'object')
        ...
        : undefined;
    const contextQuadrant = action.context?.plannerQuadrant;
    const dbQuadrant = isPlannerMatrixQuadrant(row?.planner_quadrant)
        ? row.planner_quadrant
        : (isPlannerMatrixQuadrant(action.plannerQuadrant) ? action.plannerQuadrant : undefined);
    const plannerQuadrant = dbQuadrant || (isPlannerMatrixQuadrant(contextQuadrant) ? contextQuadrant : undefined);
    ...
    return {
        ...
        context: {
            ...(action.context || {}),
            ...(plannerQuadrant ? { plannerQuadrant } : {}),
        },
...
const DEFAULT_USER_PROFILE: UserProfile = {
    ...
    onboardingPushPromptedAt: null,
    betaProgramCode: null,
    betaProgramLabel: null,
    betaProgramStartedAt: null,
    betaProgramEndsAt: null,
    betaProgramLastCheckInDate: null,
    betaProgramCheckInCount: 0,
    betaProgramDaysTarget: 0,
    betaRewardPending: false,
    betaRewardShownAt: null,
    betaRewardPayload: null,
...
export interface GameContextType {
    ...
    /**
     * O acumulado da RODADA — o mesmo papel do cycleExpBonus para quem joga sem
     * ciclo aberto. Os dois somam exp_deposited; muda o pote e quem paga no fim.
     */
    roundExpBonus: number;
    ...
    donateItem: (instanceId: string, recipientId: string) => Promise<boolean>;
...
export const GameProvider: React.FC<{ children: ReactNode, session: Session | null }> = ({ children, session }) => {
    ...
    const betaProgramCheckInRunKeyRef = useRef<string | null>(null);
    ...
    /**
     * Doa um item para um amigo. Irreversivel.
     *
     * O botao existia ha tempos e era `alert("(Simulação)")` — nunca moveu nada.
     * A troca acontece no servidor porque quem doa nao pode ser quem confere se a
     * amizade existe, nem se o item e mesmo dele.
     *
     * Nao ha desfazer: presente que pode ser retomado nao e presente, e a
     * possibilidade de retomar seria a proxima alavanca de chantagem.
     */
    const donateItem = async (instanceId: string, recipientId: string): Promise<boolean> => {
        const { data, error } = await withLatencyToast<{ data: any, error: any }>(
            supabase.rpc('donate_item_to_friend', {
        ...
        if (error) {
            const motivo = String(error.message || '');
            showToast(
                motivo.includes('NOT_FRIENDS') ? 'Você só pode doar para amigos.'
                    : motivo.includes('ITEM_EQUIPPED') ? 'Desequipe o item antes de doar.'
                        : motivo.includes('ITEM_NOT_OWNED') ? 'Esse item não está mais com você.'
                            : motivo.includes('DONATE_TO_SELF') ? 'Não dá para doar para você mesmo.'
                                : 'Não foi possível doar o item.',
                'error',
            );
            return false;
        }
    ...
    useEffect(() => {
        ...
        const loadDataFromSupabase = async () => {
            ...
            if (!levelsError && !hasPersistedAssetLevels) {
                setUserProfile((prev) => (
                    prev.lastLevelUpdate
                        ? { ...prev, lastLevelUpdate: null }
                        : prev
                ));
                ...
                supabase
                    ...
                    .then(({ error }) => {
                        if (error) {
                            console.error('Failed to clear inconsistent last_level_update:', error.message, error.details, error.hint);
                        }
                    });
            }
    ...
    const mapCompetitionProposalRow = (row: any): RelationshipCompetitionProposal => ({
        ...
        sourceArenaId: row.source_arena_id,
        arenaSnapshot: row.arena_snapshot ?? null,
        status: row.status,
        createdAt: row.created_at,
        respondedAt: row.responded_at ?? null,
    ...
    const fetchRelationshipHubData = async () => {
        ...
        if (linkIds.length > 0) {
            ...
            if (competitionLinkIds.length > 0) {
                ...
                // A proposta e o estado ANTES do duelo: nada nasceu ainda em conta
                // nenhuma. So as pendentes interessam ao card.
                const proposalsResult = await supabase
                    .from('relationship_competition_proposals')
                    .select('*')
                    .in('relationship_link_id', competitionLinkIds)
                    .eq('status', 'pending')
                    .order('created_at', { ascending: false });
                ...
                if (proposalsResult.error) {
                    console.error('Error fetching competition proposals:', proposalsResult.error);
                } else {
                    competitionProposals = (proposalsResult.data || []).map(mapCompetitionProposalRow);
                }
            }
    ...
    const createCompetitionChallenge = async (relationshipLinkId: string, sourceArenaId: string): Promise<Arena | null> => {
        ...
        // Duelo nao cobra mais (quem pagou foi o vinculo) e nao cria mais arena na
        // hora: agora e uma PROPOSTA. As arenas espelhadas so nascem quando o
        // outro aceita, porque criar arena na conta de alguem sem perguntar era
        // exatamente o que derrubou a mentoria colaborativa em agosto.
        const { data, error } = await supabase.rpc('propose_competition_challenge', {
            p_relationship_link_id: relationshipLinkId,
            p_source_arena_id: sourceArenaId,
        });
    ...
    /**
     * Aceitar, recusar ou cancelar uma proposta de duelo.
     *
     * So no aceite as duas arenas espelhadas nascem. A arena de quem aceita
     * aparece na proxima sincronizacao do hub, que e como a do oponente ja
     * aparecia antes — o fluxo antigo tambem so inseria localmente a arena de
     * quem forjava.
     */
    const respondCompetitionChallenge = async (
        proposalId: string,
        action: 'accept' | 'decline' | 'cancel',
    ): Promise<boolean> => {
        const { data, error } = await supabase.rpc('respond_competition_challenge', {
        ...
        const status = String((data as any)?.status || action);
        if (status === 'accepted') {
            showToast('Duelo aceito. O snapshot foi selado nos dois lados.', 'success');
        } else if (status === 'declined') {
            showToast('Duelo recusado.', 'info');
        } else {
            showToast('Proposta cancelada.', 'info');
        }
    ...
    const updateUserProfile = (profileData: Partial<UserProfile>) => {
        ...
        if (supabaseUserId) {
            ...
            const allowedKeys: (keyof UserProfile)[] = [
                ...
                'betaProgramCode',
                'betaProgramLabel',
                'betaProgramStartedAt',
                'betaProgramEndsAt',
                'betaProgramLastCheckInDate',
                'betaProgramCheckInCount',
                'betaProgramDaysTarget',
                'betaRewardPending',
                'betaRewardShownAt',
                'betaRewardPayload',
                'legacyProjectionSceneCredits',
    ...
    useEffect(() => {
        const userId = getSupabaseUserId();
        if (!userId || !isUuid(userId) || !isProfileLoaded || !hasHydratedFromSupabase) return;

        const todayKey = `${userId}:${getOperationalDateString()}`;
        if (betaProgramCheckInRunKeyRef.current === todayKey) return;
        betaProgramCheckInRunKeyRef.current = todayKey;

        let cancelled = false;

        (async () => {
            const result = await SupabaseService.processBetaProgramCheckIn(userId);
            if (cancelled || !result.success || !result.programKey) return;

            updateUserProfile({
                betaProgramCode: result.programKey || null,
                betaProgramLabel: result.programLabel || null,
                betaProgramStartedAt: result.startedAt || null,
                betaProgramEndsAt: result.endsAt || null,
                betaProgramLastCheckInDate: result.checkInDate || null,
                betaProgramCheckInCount: result.checkInCount ?? 0,
                betaProgramDaysTarget: result.targetDays ?? 0,
                betaRewardPending: Boolean(result.rewardPending),
                betaRewardPayload: result.rewardPayload || null,
            });

            if (!result.newCheckIn) return;

            const nextCount = Number(result.checkInCount || 0);
            const targetDays = Number(result.targetDays || 14);
            ...
            const newMessage: OracleMessage = {
                ...
                content,
                mode: oraclePreferences?.activeMode || 'neutro',
                deliveryType: 'chat',
                contextSnapshot: {
                    ...
                    betaProgram: {
                        ...
                        rewardGrantedNow: Boolean(result.rewardGrantedNow),
                    },
                },
                read: false,
                createdAt: new Date().toISOString(),
    ...
    const addAction = async (actionData: Omit<Action, 'id'>): Promise<Action> => {
        ...
        try {
            ...
            const actionPayload = {
                ...
                planner_quadrant: contextPayload?.plannerQuadrant || null,
    ...
    const updateAction = (actionId: string, actionData: Partial<Action>) => {
        ...
        if (userId) {
            ...
            if (actionData.context !== undefined || actionData.scheduledDays !== undefined || actionData.scheduledStartTime !== undefined) {
                const mergedContext = mergeScheduleIntoContext(actionData.context, actionData.scheduledDays, actionData.scheduledStartTime);
                updatePayload.context = mergedContext;
                updatePayload.planner_quadrant = mergedContext?.plannerQuadrant || null;
            }
    ...
    const searchClans = async (query: string): Promise<Clan[]> => {
        ...
        const clansWithMembers = (data || [])
            .filter((row: any) => Array.isArray(row.clan_members) && row.clan_members.length > 0)
            .map((row: any) => {
                const { clan_members: _members, ...clanRow } = row;
                return clanRow;
            });
        return (mapToCamelCase(clansWithMembers) as Clan[]).slice(0, 10);
    };
