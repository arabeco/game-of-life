-- Anonymous aggregate only. No participant identities or individual results exposed.
create table if not exists public.daily_comparison_exclusions (
  user_id uuid primary key references auth.users(id) on delete cascade
);
alter table public.daily_comparison_exclusions enable row level security;
revoke all on public.daily_comparison_exclusions from anon, authenticated;
grant all on public.daily_comparison_exclusions to service_role;
...
begin
  ...
  select case when size >= 100 then jsonb_build_object(
