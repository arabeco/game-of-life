param(
    [string]$OutputDir = "C:\Users\Afonso\Downloads\GOL1.006\marketing\vitrine-04-forja-ouro\slides",
    [string]$Screen1Path = "C:\Users\Afonso\Downloads\GOL1.006\marketing\round 4\loja.jpeg",
    [string]$Screen2Path = "C:\Users\Afonso\Downloads\GOL1.006\marketing\round 4\codexes.jpeg"
)
...
Draw-CenterText -Graphics $graphics -Text "Forja" -Font $watermarkFont -Brush $goldWashBrush -X 84 -Y 132 -Width 912 -Height 180
Draw-CenterText -Graphics $graphics -Text "Seu esfor${ccedilla}o paga`na sua evolu${ccedilla}${atilde}o." -Font $heroTitleFont -Brush $goldBrushSlide -X 134 -Y 332 -Width 812 -Height 246
Draw-CenterText -Graphics $graphics -Text "No GLYPH, execu${ccedilla}${atilde}o vira ouro.`nE ouro vira arsenal, loja e codexes`ncom valor real dentro do sistema." -Font $bodyFont -Brush $silverBrushSlide -X 142 -Y 652 -Width 796 -Height 196
Draw-Pill -Graphics $graphics -Text "Produto 04" -Font $bodyBoldFont -X 292 -Y 858 -Width 224 -Height 52
Draw-Pill -Graphics $graphics -Text "Forja" -Font $bodyBoldFont -X 536 -Y 858 -Width 188 -Height 52
...
Draw-CenterText -Graphics $graphics -Text "Aqui progresso gera`nmoeda. E moeda`ncria poder." -Font $titleLargeFont -Brush $goldBrushSlide -X 196 -Y 384 -Width 688 -Height 236
Draw-CenterText -Graphics $graphics -Text "Const${acirc}ncia alimenta uma economia vis${iacute}vel.`nOuro, fragmentos e itens transformam rotina`nem recompensa dentro do jogo." -Font $bodyFont -Brush $silverBrushSlide -X 176 -Y 676 -Width 728 -Height 236
Draw-CenterText -Graphics $graphics -Text "Executar rende." -Font $titleSmallFont -Brush $whiteBrush -X 174 -Y 936 -Width 732 -Height 84
...
for ($proofPage = 0; $proofPage -lt $proofPageCount; $proofPage++) {
    ...
    $graphics = $canvas.Graphics
    Draw-BackgroundBase -Graphics $graphics -BackgroundPath $bgSapphire -Width $width -Height $height -Tone "safira"
    Draw-CenterText -Graphics $graphics -Text "Loja, ouro e Codexes." -Font $titleLargeFont -Brush $goldBrushSlide -X 150 -Y 150 -Width 780 -Height 110

    if ($currentPair.Count -ge 2) {
        $leftPath = $currentPair[0]
        $rightPath = $currentPair[1]
        Draw-PhoneSlot -Graphics $graphics -X 164 -Y 318 -Width $proofPhoneSlotWidth -Height 560 -ImagePath $leftPath -Title (Get-PrintLabel -Path $leftPath) -Subtitle "Tela real da economia" -TitleFont $titleSmallFont -SubtitleFont $bodySmallFont -TitleBrush $goldBrushSlide -SubtitleBrush $mutedBrush -LogoPath $logoPath
        Draw-PhoneSlot -Graphics $graphics -X 656 -Y 318 -Width $proofPhoneSlotWidth -Height 560 -ImagePath $rightPath -Title (Get-PrintLabel -Path $rightPath) -Subtitle "Tela real da economia" -TitleFont $titleSmallFont -SubtitleFont $bodySmallFont -TitleBrush $goldBrushSlide -SubtitleBrush $mutedBrush -LogoPath $logoPath
        ...
        Draw-PhoneSlot -Graphics $graphics -X 393 -Y 290 -Width 294 -Height 640 -ImagePath $singlePath -Title (Get-PrintLabel -Path $singlePath) -Subtitle "Tela real da economia" -TitleFont $titleSmallFont -SubtitleFont $bodySmallFont -TitleBrush $goldBrushSlide -SubtitleBrush $mutedBrush -LogoPath $logoPath
...
Draw-CenterText -Graphics $graphics -Text "Produto 04  |  Forja e economia de ouro" -Font $eyebrowFont -Brush $eyebrowBrush -X 174 -Y 1060 -Width 732 -Height 28
...
Draw-CenterText -Graphics $contactGraphics -Text "PRODUTO 04  |  FORJA E ECONOMIA DE OURO" -Font ([System.Drawing.Font]::new($headlineFamily, 44, [System.Drawing.FontStyle]::Bold, [System.Drawing.GraphicsUnit]::Pixel)) -Brush $sheetGold -X 180 -Y 42 -Width 1240 -Height 60
