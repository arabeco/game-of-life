param(
    [string]$OutputDir = "C:\Users\Afonso\Downloads\GOL1.006\marketing\choque-01-vida-baguncada\slides",
...
$sourceDir = Join-Path $root "marketing\choque 1-5\1"
...
$slides = @(
    "Sua vida`nestá toda`nbagunçada?",
    "Tem frente demais`naberta dentro`nda sua cabeça.",
    "O dia começa`ne você já se sente`natrasado por dentro.",
    "Você resolve`no que grita.`nNão o que importa.",
    "No fim da noite,`ntudo ainda`nparece espalhado.",
    "Bagunça crônica`nnão é destino.`nÉ falta de sistema.",
    "Sua vida`nnão precisa`ncontinuar assim."
)

& $template -OutputDir $OutputDir -BackgroundPaths $backgrounds -Slides $slides -SheetTitle "Choque 01 - Vida baguncada" -Theme "Crimson" -TextStyle $TextStyle -FontChoice $FontChoice
